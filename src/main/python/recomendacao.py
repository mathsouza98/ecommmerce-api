...
def recommend_products(user_id: str=None):
    ...
    if (user_id):
        ...
        final_user_rating = user_history.pivot_table(index='productId', columns='userId', values='visualization', fill_value=0)
        ...
        productId = user_history[user_history.userId == user_id].sort_values('visualization').tail(1).productId.item()
